...
const nodeRestClient = function(options) {
  ...
  const ioFacade = function(parserManager, serializerManager) {
    ...
    const errorContext = (parserFn) => function() {
      try {
    ...
    result.parsers.remove = errorContext(parserManager.remove).bind(parserManager);
    result.parsers.find = errorContext(parserManager.find).bind(parserManager);
    result.parsers.getAll = errorContext(parserManager.getAll).bind(parserManager);
    result.parsers.getDefault = errorContext(parserManager.getDefault).bind(parserManager);
    result.parsers.clean = errorContext(parserManager.clean).bind(parserManager);

    // serializers facade
    result.serializers.add = errorContext(serializerManager.add).bind(serializerManager);
    result.serializers.remove = errorContext(serializerManager.remove).bind(serializerManager);
    result.serializers.find = errorContext(serializerManager.find).bind(serializerManager);
    result.serializers.getAll = errorContext(serializerManager.getAll).bind(serializerManager);
    result.serializers.getDefault = errorContext(serializerManager.getDefault).bind(serializerManager);
    result.serializers.clean = errorContext(serializerManager.clean).bind(serializerManager);
...
class ConnectManager {
  ...
  configureRequest(req, config, clientRequest) {
    if (config.timeout) {
      req.setTimeout(config.timeout, () => clientRequest.emit('requestTimeout', req));
    }
  ...
  handleResponse(res, data, callback) {
    ...
    // found
    this.#parserManager.get(res).parse(data, this.clientEmitterWrapper(this.#client), (parsedData) => callback(parsedData, res));
  };
...
const innerDebug = function() {
  ...
  const now = new Date();
  const header = now.getHours() + ':' + now.getMinutes() + ':' + now.getSeconds() + ' [NRC CLIENT]' + innerDebug.caller.name + ' -> ';
