...
exports.Client = function (options){
	...
	var ConnectManager = {
		"xmlctype": new RegExp("^application/xml(;\\s*[\\w-]+=[\\w-]+)*$", "i"),
		"jsonctype": new RegExp("^application/json(;\\s*[\\w-]+=[\\w-]+)*$", "i"),
		"isXML":function(content){
			var result = false;
			if (!content) return result;

			result = this.xmlctype.test(content)
		...
		"isJSON":function(content){
			...
			result = this.jsonctype.test(content)
