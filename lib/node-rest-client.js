...
exports.Client = function (options){
    ...
    //connection manager
    connectManager = new ConnectManager(parserManager, serializerManager),
    //declare util constants
    CONSTANTS={
        HEADER_CONTENT_LENGTH:"Content-Length"
    },
    //error execution context
    errorContext = function(logic){
        return function(){
            try{
    ...
    // containers for methods, parsers y serializers
    this.methods={};
    this.parsers={};
    this.serializers={};
        ...
        connect : function(method, url, args, callback, clientRequest){
            ...
            if (typeof args === 'function'){
                ...
                if (args.parameters && Object.keys(args.parameters).length > 0){
                  // validate URL consistency, and fix it adding query parameter separation char

                  // check if URL already has '?' path parameter separator char in any position that is not final
                  // if true throw error
               ...
               options.path +=(options.path.charAt(pathLength-1) === '?'?"":"?");
                  //check if we have serializable parameter container, that must be serialized and encoded
    ...
    // parsers facade
    this.parsers.add = errorContext(parserManager.add);
    this.parsers.remove = errorContext(parserManager.remove);
    this.parsers.get = errorContext(parserManager.get);
    this.parsers.getAll = errorContext(parserManager.getAll);
    this.parsers.clean = errorContext(parserManager.clean);
    
    //serializers facade
    this.serializers.add = errorContext(serializerManager.add);
    this.serializers.remove = errorContext(serializerManager.remove);
    this.serializers.get = errorContext(serializerManager.get);
    this.serializers.getAll = errorContext(serializerManager.getAll);
    this.serializers.clean = errorContext(serializerManager.clean);
...
var ConnectManager = function(parserManager, serializerManager) {
    ...
    this.handleResponse = function(res,data,callback){
        ...
        parserManager.get(res).parse(data, this.emit, function(parsedData){
    ...
    this.proxy = function(options, callback){
        ...
        debug("proxy options",options.proxy);
            ...
            proxyTunnel.on('connect',function(res, socket, head){
                ...
                //find valid serializer to be used to serialize request data, first one found
                // is the one to be used.if none found for match condition, default serializer is used
               if(options.data){
                    serializerManager.get(options).serialize(options.data, this.emit, function(serializedData){
                        request.write(serializedData);
                        request.end(); // end request when data is written
                    });
                }else{
                    // no data to be serialized end request
